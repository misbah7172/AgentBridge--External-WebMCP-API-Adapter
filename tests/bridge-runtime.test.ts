...
const loadBridge = async (cartItems: unknown[] = []) => {
...
describe("injected WebMCP bridge", () => {
  it("registers only safe base tools for an empty cart", async () => {
    const tools = await loadBridge();
    expect(tools.map((tool) => tool.name)).toContain("add_to_cart");
    expect(tools.map((tool) => tool.name)).not.toEqual(expect.arrayContaining(["update_cart", "remove_from_cart", "apply_coupon", "checkout"]));
  });
  it("registers populated-cart tools after cart discovery", async () => {
    const tools = await loadBridge([{ id: "item-1" }]);
